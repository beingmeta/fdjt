        ...
        */
        ...
        function CodexLayout(init){
            ...
            function addContent(root,timeslice,timeskip,
                ...
                var blocks=[], terminals=[], blockinfo=[], styles=[];
                ...
                // synchronously
                gatherBlocks(root,root,blocks,terminals,blockinfo,styles);
                ...
                function step(){
                    var block=blocks[ni], style=styles[ni];
                    var info=blockinfo[ni], terminal=terminals[ni]||false;
                    var next=blocks[ni+1], nextinfo=blockinfo[ni+1];
                    var tracing=false;
                        ...
                        // which extends below the bottom of the page
                           ...
                           block=newPage(block);
                            ...
                            if ((split)&&(split!==block)) {
                                blocks[ni]=split;
                                styles[ni]=style=getStyle(split);
                                blockinfo[ni]=getBlockInfo(split,style);}
                            else {
                ...
                // blocks within them)
                function gatherBlocks(root,node,blocks,terminals,info,styles,style){
                    ...
                    if (((atomic)&&(atomic.match(node)))||
                        ...
                        if ((node.offsetHeight===0)||
                            ...
                            info.push(getBlockInfo(node,style));
                    ...
                    if ((node.tagName!=="BR")&&
                        ...
                        if ((disp==='block')||(disp==='table')||
                            ...
                            while (i<len) {
                                var ch=children[i++];
                                if (ch.nodeType===1)
                                    gatherBlocks(root,ch,blocks,terminals,info,styles);}
                        ...
                        while (j<n_elts) {
                            ...
                            if (cstyle.display!=='inline')
                                gatherBlocks(root,child,blocks,terminals,styles,info,cstyle);}
                        moveUp(node);}
                    else {}}
