...
fdjt.SimpleCodex=
    (function(){
        ...
        function SimpleCodex(root,opts){
            if (!(this instanceof SimpleCodex))
                return new SimpleCodex(root,opts);
            if (!(opts)) opts={};
            addClass(root,"simplecodex");
            this.selector=opts.blocks;
            this.start=root.firstChild;
            return this;}

        SimpleCodex.prototype.clear=function(){
            var shown=getChildren(this.root,this.selector);
            dropClass(toArray(shown),"simplecodexshown");};
        
        SimpleCodex.prototype.setStart=function(child){
            var root=this.root, last=child, scan=child.nextElementSibling;
            var h=root.offsetHeight;
            this.clear(); this.start=child; 
            addClass(child,"simplecodexshown");
            while ((scan)&&(root.scrollHeight<h)) {
                if (scan.nodeType===1) addClass(scan,"simplecodexshown");
                if (root.scrollHeight>h) {
                    dropClass(scan,"simplecodexshown");
                    this.end=last;
                    return;}
                last=scan;
        ...
        SimpleCodex.prototype.setEnd=function(child){
            var root=this.root, last=child, scan=child.nextElementSibling;
            var h=root.offsetHeight;
            this.clear(); this.end=child; 
            addClass(child,"simplecodexshown");
            while ((scan)&&(root.scrollHeight<h)) {
                if (scan.nodeType===1) addClass(scan,"simplecodexshown");
                if (root.scrollHeight>h) {
        ...
        SimpleCodex.prototype.pageForward=function(){
            var end=this.end;
            if (end.nextElementSibling) {
                this.setStart(end.nextElementSibling);
                return true;}
            else return false;};

        SimpleCodex.prototype.pageBackward=function(){
            var start=this.start;
            if (start.previousElementSibling) {
                this.setEnd(start.previousElementSibling);
                return true;}
            else return false;};
